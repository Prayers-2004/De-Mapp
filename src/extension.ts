		...
		vscode.commands.registerCommand(commands.PluginCommit, async () => {
			...
			try {
				...
				await vscode.window.withProgress({
					...
					title: "Summarizing Chat History",
					cancellable: false
				}, async (progress) => {
				...
				if (result === "Open File") {
					// Open the summarize.json file
					let activeController = WebviewProvider.getActiveInstance()?.controller
					if (!activeController) {
						activeController = WebviewProvider.getVisibleInstance()?.controller
					}
					if (!activeController) {
						activeController = WebviewProvider.getSidebarInstance()?.controller
					}
